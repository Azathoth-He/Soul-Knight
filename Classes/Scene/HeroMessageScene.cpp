...
Scene* HeroMessage::createScene(const std::string hero)
{
	
	return HeroMessage::create(hero);
}

HeroMessage* HeroMessage::create(const std::string hero)
{
	HeroMessage* layer = new(std::nothrow)HeroMessage;
	if (layer && layer->init(hero))
	{
		layer->autorelease();
		return layer;
	}
...
bool HeroMessage::init(const std::string hero)
{
	if (!Scene::init())
	{
		return false;
	...
	heroName = hero;
	myMoney = 10000;
	auto _hero = Hero::create(heroName);
	myHero = _hero;
...
{
	if (myMoney >= myHero->getUpLevelMoney())
	{
